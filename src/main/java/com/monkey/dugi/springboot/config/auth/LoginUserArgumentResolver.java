...
public class LoginUserArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public boolean supportsParameter(MethodParameter parameter) { // 1. 컨트롤러 메서드의 특정 파라미터를 지원하는지 판단
                                                                  // 2. 여기서는 파라미터에 @LoginUser 어노테이션이 붙어 있고, 파라미터 클래스 타입이
                                                                  //     SessionUser.class인 경우 true 반환
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
                                  NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception { // 1. 파라미터에 전달할 객체 생성
                                                                                                                      // 2. 여기서는 세션에서 객체를 가져옴
