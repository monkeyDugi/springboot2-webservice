...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT) // 랜덤 포트 환경 설정
public class PostsApiControllerTest {
    ...
    @Test
    @WithMockUser(roles = "USER") // 1. 인증된 가짜 사용자를 만들어서 사용
                                  // 2. roles 에 권한을 추가할 수 있음
                                  // 3. 즉, 이 어노테이션으로 인해 ROLE_USER 권한을 가진 사용자가 API를 요청하는 것과 동일한 효과를 가지게 됨.
    public void Posts_등록된다() throws Exception {
        ...
        mvc.perform(post(url)
                ...
                // body 영역은 문자열로 표현하기 위해 ObjectMapper를 통해 문자열 JSON으로 반환
